<x-app-layout>
    <x-slot name="page_title">{{ $page_title ?? 'Purchase Create |' }}</x-slot>
        ...
        <div class="row">
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        <div class="row mb-2">
                            <form action="{{ url('admin-panel/dashboard/purchases') }}" method="POST" enctype="multipart/form-data">
                                @csrf
    ...
    <x-slot name="script">
        ...
        <script type="text/javascript">
            ...
            function set_product_in_order_table(product_id) {
                ...
                .then(function (response) {
                    ...
                    newRow.innerHTML = `
                        ...
                        <td>${product.name} <input type="text" name="products[${product.id}][id]" value="${product.id}" hidden></td>
                        <td>${product.purchase_price}</td>
                        <td><input type="number" min="0" name="products[${product.id}][quantity]" value="1" id="product${product.id}_quantity" onchange="calculatePurchasePrice(${product.id}, ${product.purchase_price})"></td>
                        <td id="product${product.id}_discount">${product.discount || 0.00}</td>
                        <td class="total_tax_price" id="product${product.id}_tax">${product.tax || 0.00}</td>
                        <td class="total_purchase_price" id="product${product.id}_total_purchase_price">${product.purchase_price}</td>
            ...
            $(document).ready(function () {
                if($("#is_promotion").val() === "partial" || $("#is_promotion").val() === "paid") {
